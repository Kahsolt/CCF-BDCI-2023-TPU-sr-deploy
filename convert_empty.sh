...
cd models
if [ ! -d empty ]; then
  echo ">> please run: python model_empty.py"
  exit -1
fi
cd empty
...
MODEL_NAME=empty
MODEL_PATH=empty.pt
MLIR_NAME=empty.mlir
BMODEL_FP32_FILE=empty.fp32.bmodel
BMODEL_INT8_FILE=empty.int8.bmodel
BMODEL_CALIB_FILE=empty.int8.cali
